...
function onSelect() {        
    if (reticle.visible) {
        ...
        ////////////////////
            ...
            /////////////////////////
        // Create a snowflake shape using a hexagonal pattern
        const shape = new THREE.Shape();
        const length = 0.7; // Length of the snowflake arms

        // Create the center of the snowflake
        shape.moveTo(0, 0);
        shape.lineTo(length, 0); // First arm

        // Add branches to the snowflake
        shape.moveTo(0, 0);
        shape.lineTo(-length, 0); // Second arm

        shape.moveTo(0, 0);
        shape.lineTo(0, length); // Third arm

        shape.moveTo(0, 0);
        shape.lineTo(0, -length); // Fourth arm

        // Add the "branches" of the snowflake (you can play around with the branches and angles)
        const extrudeSettings = {
            steps: 1,
            depth: 0.05, // Depth of the snowflake
            bevelEnabled: true,
            bevelThickness: 0.02,
            bevelSize: 0.06,
            bevelSegments: 1
        ...
        const material = new THREE.MeshStandardMaterial({ color: 0x00ffff, metalness: 0.1, roughness: 0.8 });
